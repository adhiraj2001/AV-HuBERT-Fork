...
class AVHubertDataset_2(FairseqDataset):
    def __init__(
            self,
            # manifest_path: str,
        ...
        self.label_id = 'test-0'
        self.label_list = [['DUMMY']]
    ...
    def __getitem__(self, index):
        frames = self.transform(self.frames)
        frames = np.expand_dims(frames, axis=-1)
        frames = torch.from_numpy(frames.astype(np.float32))
        return {"id": 1, 'fid': 1, "video_source": frames, 'audio_source': None, "label_list": self.get_labels(index)}
...
class AVHubertDataset_3(FairseqDataset):
    def __init__(
            self,
    ...
    def load_audio(self, wav, sr):
        ...
        feats = logfbank(wav, samplerate=sr).astype(np.float32) # [T, F]
        feats = stacker(feats, self.stack_order_audio) # [T/stack_order_audio, F*stack_order_audio]
        feats = torch.from_numpy(feats.astype(np.float32))
    ...
    def __getitem__(self, index):
        video_feats = self.load_video(self.video_frames) if self.video_frames is not None else None
        audio_feats = self.load_audio(self.audio_wav, self.sample_rate) if self.audio_wav is not None else None
